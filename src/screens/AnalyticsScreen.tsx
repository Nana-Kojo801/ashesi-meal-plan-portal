...
import { TrendingUp, TrendingDown, Coffee, ShoppingBag, Flame, Star, Percent, Leaf } from 'lucide-react';
...
const RED = '#D81E2C';
const RED_SOFT = 'rgba(216,30,44,.55)';
...
function ChartTooltip({ active, payload, label, prefix = 'GHS ' }: {
  ...
  return (
    <div style={{
      ...
      <div style={{ color: RED }}>{prefix}{payload[0].value.toFixed(0)}</div>
    </div>
  );
}
...
function StatCard({ icon, label, value, sub, delay = 0, accent = false }: {
  ...
  return (
    <motion.div
      ...
      style={{
        background: '#fff', border: `1px solid ${accent ? 'rgba(216,30,44,.2)' : '#ECE0D4'}`,
    ...
    >
      ...
      <div style={{
        ...
        fontSize: 26, fontWeight: 700, letterSpacing: '-0.8px', color: accent ? RED : '#1C1413',
        lineHeight: 1,
      }}>{value}</div>
...
function ChartCard({ title, children, delay = 0 }: {
  ...
  return (
    ...
    >
      <div style={{ fontWeight: 800, fontSize: 14, marginBottom: 18 }}>{title}</div>
      {children}
    </motion.div>
  );
}
...
export function AnalyticsScreen({ studentId, balanceData, isMobile: _isMobile }: AnalyticsScreenProps) {
  const endDate   = todayStr();
  const startDate = nDaysAgo(59); // 60 days covers current + last full month
  ...
  const stats = useMemo(() => {
    ...
    // fill all 30 days (0 if no spending)
    ...
    // item stats
    const itemMap: Record<string, { qty: number; spend: number }> = {};
    last30.forEach(tx => {
      if (!itemMap[tx.name]) itemMap[tx.name] = { qty: 0, spend: 0 };
    ...
    const mostExpensive = [...items].sort((a,b) => b.spend - a.spend)[0] ?? null;
    ...
    const thisMonth  = history.filter(tx => tx.date.slice(0,10) >= thisStart && tx.date.slice(0,10) <= thisEnd).reduce((s,tx) => s + txCost(tx), 0);
    const lastMonth  = history.filter(tx => tx.date.slice(0,10) >= lastStart && tx.date.slice(0,10) <= lastEnd).reduce((s,tx) => s + txCost(tx), 0);

    // savings streak (consecutive days of zero spend going back from today)
    let streak = 0;
    for (let i = 0; i < 30; i++) {
      const d = nDaysAgo(i);
      if ((byDay[d] ?? 0) > 0) break;
      streak++;
    }

    // avg daily vs limit
    const activeDays  = Object.values(byDay).filter(v => v > 0).length || 1;
    const avgDaily    = last30.reduce((s,tx) => s + txCost(tx), 0) / 30;
    const dailyLimit  = balanceData?.daily_spending_limit ?? 0;
    const limitPct    = dailyLimit > 0 ? Math.round((avgDaily / dailyLimit) * 100) : null;

    // variety
    const varietyScore = new Set(last30.map(tx => tx.name)).size;

    return {
      dailyTrend, cafes, items, mostFrequent, mostExpensive,
      dayOfWeek, morning, afternoon, evening, todTotal,
      thisMonth, lastMonth,
      streak, avgDaily, limitPct, varietyScore,
    ...
    return (
      <div style={{ textAlign: 'center', padding: '60px 20px', background: '#fff', border: '1px dashed #ECE0D4', borderRadius: 20 }}>
        ...
        <div style={{ fontSize: 13, color: '#7A6A63', fontWeight: 600, marginTop: 6 }}>
          Make a few more purchases and check back — analytics will appear once you have enough history.
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
      ...
      {/* ── daily trend ─────────────────────────────────────────────────── */}
      <ChartCard title="Daily spending trend" delay={0.05}>
        <ResponsiveContainer width="100%" height={180}>
          <LineChart data={stats.dailyTrend} margin={{ top: 4, right: 4, left: -18, bottom: 0 }}>
            ...
            <Line
              ...
              dot={false} activeDot={{ r: 5, fill: RED, stroke: '#fff', strokeWidth: 2 }}
            />
          </LineChart>
        </ResponsiveContainer>
      </ChartCard>
      ...
      {/* ── cafe ranking ─────────────────────────────────────────────────── */}
      <ChartCard title="Top cafés / vendors" delay={0.1}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {stats.cafes.map((cafe, i) => {
            const maxSpend = stats.cafes[0].spend || 1;
              ...
              <div key={cafe.name}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 5 }}>
                  <div style={{ fontSize: 13, fontWeight: 700, display: 'flex', alignItems: 'center', gap: 7 }}>
                    <span style={{
                      ...
                      background: i === 0 ? RED : '#F2E7DC',
                      color: i === 0 ? '#fff' : '#7A6A63',
                      display: 'inline-flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0,
                    }}>{i + 1}</span>
                    {cafe.name}
                  </div>
                  <div style={{ fontSize: 12.5, fontWeight: 700, color: '#7A6A63' }}>
                    GHS {cafe.spend} · {cafe.visits} visit{cafe.visits !== 1 ? 's' : ''}
                  </div>
                </div>
                <div style={{ height: 6, borderRadius: 99, background: '#F2E7DC', overflow: 'hidden' }}>
                  <motion.div
                    initial={{ width: 0 }}
                    animate={{ width: `${pct}%` }}
                    transition={{ duration: 0.8, delay: 0.15 + i * 0.06, ease: [0.4, 0, 0.2, 1] }}
      ...
      {/* ── day of week ──────────────────────────────────────────────────── */}
      <ChartCard title="Spending by day of week" delay={0.14}>
        <ResponsiveContainer width="100%" height={150}>
          <BarChart data={stats.dayOfWeek} margin={{ top: 4, right: 4, left: -18, bottom: 0 }}>
            ...
            <YAxis tick={{ fontSize: 10, fill: '#7A6A63', fontWeight: 600 }} tickLine={false} axisLine={false} tickFormatter={v => `${v}`} />
            <Tooltip content={<ChartTooltip />} />
            <Bar dataKey="spend" radius={[6, 6, 0, 0]}>
              {stats.dayOfWeek.map((entry, i) => {
                const maxSpend = Math.max(...stats.dayOfWeek.map(d => d.spend)) || 1;
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 14 }}>
        <StatCard
          icon={<Flame size={14} />} label="Most visited"
          value={stats.cafes[0]?.name?.split(' ')[0] ?? '—'}
          sub={`${stats.cafes[0]?.visits ?? 0} visits · GHS ${stats.cafes[0]?.spend ?? 0}`}
          delay={0.18}
        />
        <StatCard
          icon={<Star size={14} />} label="Fav item"
          value={stats.mostFrequent?.name?.split(' ').slice(0,2).join(' ') ?? '—'}
          sub={`${stats.mostFrequent?.qty ?? 0}× ordered`}
          delay={0.22}
        />
        <StatCard
          icon={<ShoppingBag size={14} />} label="Biggest splurge"
          value={stats.mostExpensive?.name?.split(' ')[0] ?? '—'}
          sub={`GHS ${Math.round(stats.mostExpensive?.spend ?? 0)} total`}
          delay={0.26}
        />
        <StatCard
          icon={<Coffee size={14} />} label="Variety"
          value={`${stats.varietyScore}`}
          sub="unique items tried"
          delay={0.30}
        />
      </div>

      {/* ── time of day ──────────────────────────────────────────────────── */}
      <ChartCard title="Time of day breakdown" delay={0.34}>
        {[
      ...
      {/* ── month comparison + streak + avg vs limit ─────────────────────── */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 14 }}>
        <motion.div
          initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.35, delay: 0.38, ease: [0.2, 0.7, 0.2, 1] }}
          style={{
            background: '#fff', border: '1px solid #ECE0D4', borderRadius: 20,
            padding: '18px 20px', boxShadow: '0 12px 26px -18px rgba(110,30,18,.18)',
          }}
        >
          ...
          <div style={{ fontSize: 11, color: '#7A6A63', fontWeight: 600, marginTop: 3 }}>
            Last month: GHS {Math.round(stats.lastMonth)}
          </div>
        </motion.div>
        ...
        <StatCard
          ...
          sub={stats.streak === 0 ? 'Spend today' : "No purchases — great!"}
          delay={0.42} accent={stats.streak >= 2}
        />

        {stats.limitPct !== null && (
          <StatCard
            icon={<Percent size={14} />} label="Avg daily vs limit"
            value={`${stats.limitPct}%`}
            sub={`GHS ${Math.round(stats.avgDaily)} avg · GHS ${balanceData?.daily_spending_limit ?? 0} limit`}
            delay={0.46} accent={stats.limitPct > 80}
          />
        )}
      </div>
    </div>
